...
//        }
    ...
    protected void onTaskNew_BtnCreateTaskAction(Component c, ActionEvent event) {
        ...
        Button clickedComponent = (Button) c;
        Form f = c.getComponentForm();

        TextField txtTaskTitle = (TextField) findByName("txtTaskTitle", f);
        Picker pickerProjectName = (Picker) findByName("pickerProjectName", f);
        Picker pickerStatus = (Picker) findByName("pickerStatus", f);
        Picker pickerDate = (Picker) findByName("pickerDate", f);
        Picker pickerTime = (Picker) findByName("pickerTime", f);
        TextArea txtDescription = (TextArea) findByName("txtDescription", f);
        TextArea txtPhysicalAddress = (TextArea) findByName("txtPhysicalAddress", f);
        Picker pickerPriority = (Picker) findByName("pickerPriority", f);
...
//        pickerTime.setType(Display.PICKER_TYPE_TIME);
        ...
        DateCustom dateCustom = new DateCustom();
        Date dateDue = dateCustom.getDateTimeObject(pickerDate, pickerTime);

        DateCustom currentDate = new DateCustom(dateDue);
        //Date not older than current
        String message = "";
        ...
        if (pickerProjectName.getSelectedString().equals("Select Project")) {
            message += "Please select a Project. ";
        }

        if (pickerPriority.getSelectedString().equals("Select Priority")) {
            message += "Please select Priority. ";
        }

        if (pickerStatus.getSelectedString().equals("Select Status")) {
            message += "Please select Task Status. ";
        }

        if (currentDate.isDateValid() == false) {
            message += "A Task can only be due from today and in future. ";
        }

        if (currentDate.isTimeValid() == false) {
            message += "A Task's Time can only be due from " + currentDate.getTimeNow() + " till Midnight (23h59). ";
        }

        if (taskUsers == null || taskUsers.isEmpty()) {
            message += "Please assign atleast one user to the task. ";
        }

        String successMessage = "Updated task.";
        String errorMessage = "Failed to update task.";

        if (message.length() == 0) {

            if (clickedComponent.getText().indexOf("Update Task") < 0) {//new Task
                taskToAdd = ParseObject.create(Const.PARSE_CLASS_TASK);
                successMessage = "Added new task.";
                errorMessage = "Failed to add new task.";
                Log.p("Adding new task", Log.DEBUG);
            } else {
                Log.p("Updating existing task", Log.DEBUG);
            }

            //Temp List<ParseObject> because Parse does not take Hashtables, it takes List "Arrays"
            java.util.List<ParseObject> list = new ArrayList<>();
            Set<String> set = taskUsers.keySet();
            for (String key : set) {
                list.add(taskUsers.get(key));
            }

            Project project = null;
            for (int i = 0; i < projects.size(); i++) {
                Project temp = projects.get(i);
                if (pickerProjectName.getSelectedString().equals(temp.getTitle())) {
                    Log.p("Task action: Found project =" + temp.getTitle(), Log.DEBUG);
                    project = temp;
                }
            }
            ...
            switch (pickerPriority.getSelectedString()) {
                case "8 Hours":
                    priority = 8;
                    break;
                case "16 Hours":
            ...
            taskToAdd.put("title", title);
            taskToAdd.put("project", project.getParseObject());
            taskToAdd.put("user", currentUser);
            taskToAdd.put("dateDue", currentDate.getDate());
            taskToAdd.put("status", pickerStatus.getSelectedString());
            taskToAdd.put("taskUsers", list);
            taskToAdd.put("taskUsers", list);
            taskToAdd.put("priority", priority);

            //Optionals
            if (txtDescription.getText().length() > 0) {
                taskToAdd.put("description", txtDescription.getText());
            }
            if (txtPhysicalAddress.getText().length() > 0) {
                taskToAdd.put("physicalAddress", txtPhysicalAddress.getText());
            }

            set = taskUsers.keySet();

            for (String strUserObj : set) {
                ParseObject obj = taskUsers.get(strUserObj);
                Log.p("Members added=" + obj.getObjectId(), Log.DEBUG);
            }

            boolean newTaskAdded = TaskService.addTask(taskToAdd);

            if (newTaskAdded == true) {

                Utility.showDialog("Success", successMessage);
                taskUsers.clear();
                DateCustom custom = new DateCustom();

                if (custom.getDateTodayStart().getTime() < currentDate.getDate().getTime()
                        && currentDate.getDate().getTime() < custom.getDateTodayEnd().getTime()) {
                    fetchTasks(current, Const.TASK_TODAY);
                } else if (custom.getDateWeekStart().getTime() < currentDate.getDate().getTime()
                        && currentDate.getDate().getTime() < custom.getDateWeekEnd().getTime()) {
                    fetchTasks(current, Const.TASK_THIS_WEEK);
                } else if (custom.getDateTodayStart().getTime() < currentDate.getDate().getTime()
                        && currentDate.getDate().getTime() < custom.getDateMonthNext30Days().getTime()) {
                    fetchTasks(current, Const.TASK_NEXT_30_DAYS);
                }
            ...
            Log.p("Error saving task: " + message, Log.DEBUG);
            Utility.showDialog("Error", message);
        }
    ...
    protected void onHelp_BtnEmailAction(Component c, ActionEvent event) {
        ...
        Display.getInstance().sendMessage(
                new String[]{"task@titconsulting.co.za"},
                "Task Management App", msg);
...
//        }
    ...
    protected void onProjectNew_BtnProjectActionAction(Component c, ActionEvent event) {
        ...
        Form f = c.getComponentForm();
        TextField txtTitle = findTxtTitle();
        TextArea txtComment = findTxtDescription();

        Picker dateTimePicker = (Picker) findByName("dateTimePicker", f);
        Picker timePicker = (Picker) findByName("timePicker", f);
        ...
        if (currentDate.isDateValid() == false) {
            message += "Projects can only be due from today and in future. ";
        }
        ...
        if (message.length() == 0) {
            ...
            ParseObject newProject = ProjectService.addProject(
                    currentUser, title, description, dateDue);

            if (newProject != null) {
                Utility.showDialog("Success", "Added new project.");
                fetchProjects(current);
                current.showBack();
            } else {
                Utility.showDialog("Error", "Failed to add new project.");
            }

        } else {
            Utility.showDialog("Error", message);
        }
        Log.p("[Statemachine.onProjectNew_BtnProjectActionAction] dsDueDate=" + dateDue, Log.DEBUG);
        Log.p("[Statemachine.onProjectNew_BtnProjectActionAction] dsDueDate long=" + dateDue.getTime(), Log.DEBUG);
    }
    ...
    protected void beforeTaskNew(Form f) {
        ...
        pickerTime.setType(Display.PICKER_TYPE_TIME);
        pickerTime.setTime(0); //00:00AM = Minutes since midnight

        newTask = f;

        Button btnCommunication = (Button) findByName("btnCommunication", f);
        btnCommunication.remove();
...
//    }
    ...
    protected void beforeProjectNew(Form f) {
        ...
        Picker timePicker = (Picker) findByName("timePicker", f);
        timePicker.setType(Display.PICKER_TYPE_TIME);
        timePicker.setTime(0); //00:00AM = Minutes since midnight
    ...
    protected void beforeTaskNewMembers(Form f) {
        ...
        analytics(f, "New Task Add Members");

        if (taskUsers == null) {
            taskUsers = new HashMap<>();
        }

        UserService service = new UserService();
        Hashtable[] users = service.getUsersHashtable();
        Container content = (Container) findByName("content", f);
        content.removeAll();

        for (Hashtable hash : users) {

            ParseUser parseUser = (ParseUser) hash.get("parseObject");
            Image btnIcon = (Image) hash.get("btnIcon");
            String lblTitle = hash.get("lblTitle").toString();

            final MultiButton mb = new MultiButton();
            mb.setUIID("MultiButtonColor");
            mb.setIcon(btnIcon);
            mb.setTextLine1(lblTitle);
            mb.setCheckBox(true);
            mb.setUIIDLine1("LabelWhite");
            mb.setUIIDLine2("LabelWhite");

            Set<String> set = taskUsers.keySet();
            for (String key : set) {
                ParseObject object = taskUsers.get(key);
            ...
            mb.addActionListener(new ActionListener() {
                ...
                public void actionPerformed(ActionEvent evt) {
                    ...
                    if (taskUsers.size() == 0) {
                        try {
                            ParseObject newObjectToAdd = ParseObject.fetch(Const.PARSE_CLASS_USER, parseUser.getObjectId());
                            taskUsers.put(parseUser.getObjectId(), newObjectToAdd);
                        } catch (ParseException e) {
                            Log.p("Error view existing task actionListner 1: " + "", Log.DEBUG);

                        }
                    }

                    Set<String> set2 = taskUsers.keySet();
                    java.util.List<ParseObject> toAdd = new ArrayList<>();
                    java.util.List<ParseObject> toRemove = new ArrayList<>();
                    for (String key2 : set2) {
                        ParseObject object1 = taskUsers.get(key2);

                        if ((!object1.getObjectId().equals(parseUser.getObjectId()))
                                && mb.isSelected() == true) {
                            try {
                                ParseObject newObjectToAdd = ParseObject.fetch(Const.PARSE_CLASS_USER, parseUser.getObjectId());
                                if (!taskUsers.containsKey(parseUser.getObjectId())) {
                                    //taskUsers.put(parseUser.getObjectId(), newObjectToAdd);
                                    toAdd.add(newObjectToAdd);
                                }
                            } catch (ParseException e) {
                                Log.p("Error view existing task actionListner 2: " + "", Log.DEBUG);

                            }
                        } else if (object1.getObjectId().equals(parseUser.getObjectId())
                                && mb.isSelected() == false) {
                            //taskUsers.remove(key2);
                            toRemove.add(object1);
                        }

                    }//end set2

                    //Step below is done like this to avoid ConcurrentModificationException
                    //to add
                    for (ParseObject p : toAdd) {
                        taskUsers.put(p.getObjectId(), p);
                    }

                    //to remove
                    for (ParseObject p : toRemove) {
                        taskUsers.remove(p.getObjectId());
                    }
        ...
        Style s = UIManager.getInstance().getComponentStyle("LabelWhite");
        TextField searchField = new TextField("", "Search"); // <1>
        searchField.getHintLabel().setUIID("TextHint");
        searchField.setUIID("TextField");
            ...
            if (t.length() < 1) {
                ...
                for (Component cmp : content) {
                    ...
                    if (cmp instanceof MultiButton) {
                        val = ((MultiButton) cmp).getTextLine1();
        ...
        Command back = new Command("Back") {
            ...
            public void actionPerformed(ActionEvent evt) {
                super.actionPerformed(evt);
                Log.p("Back selected - Selected no users = " + taskUsers.size(), Log.DEBUG);
                updateAssignedLabel();
                newTask.showBack();
            }
    ...
    protected void onTaskNewMembers_BtnCloseAction(Component c, ActionEvent event
        ...
        Log.p("Add members - Selected no users = " + taskUsers.size(), Log.DEBUG);
        updateAssignedLabel();
        newTask.showBack();
    ...
    @Override
    protected void beforeFormCommunication(final Form f) {
        analytics(f, "Task Diary");

        final Container messages = (Container) findByName("messages", f);
        ...
        for (Communication object : list) {
            ...
            Label label = isItMe(userName, email, message);

            messages.add(label);

            label.setUIID("LabelWhite");
            messages.setUIID("LabelWhite");
        }
        ...
        ParseLiveQuery.setWsCallback(new ParseLiveQuery.WsCallback() {
            ...
            public void onMessage(String var1) {
                ...
                try {
                    //Log.p("onMessage()=1 " + var1, Log.DEBUG);
                    JSONObject object = new JSONObject(var1).getJSONObject("object");
                    String message = object.getString("message");
                    String userName = object.getString("userName");
                    String email = object.getString("email");

                    //Log.p("onMessage()=2 message=" + message, Log.DEBUG);
                    //Utility.showDialog(userName + ": " + message, userName + ": " + message, userName + ": " + message);
                    ...
                    messages.scrollComponentToVisible(label);
                    //messages.setScrollY(1);
        ...
        try {
            ...
            try {
                task = ParseObject.fetch(Const.PARSE_CLASS_TASK, taskId);
                Log.p("before Live query taskId= ", Log.DEBUG);
            } catch (ParseException e) {
                e.printStackTrace();
            }
            ...
            try {
                ParseLiveQuery liveQuery = new ParseLiveQuery(query) {
                    ...
                    public void event(String op, int requestId, ParseObject object) {
                        Log.p(op + "  " + object.getObjectId(), Log.DEBUG);
                    }
                };
            } catch (NullPointerException e) {
                Log.p("Statemachine can\t subscribe to Live query, check query.whereEqualTo(\"task\", task)", Log.DEBUG);
            }
    ...
    protected void onFormCommunication_BtnSendCommentAction(Component c, ActionEvent event) {
        ...
        /*try {
            task = ParseObject.fetch(Const.PARSE_CLASS_TASK, "GPiHHEYVup");
        } catch (ParseException e) {
            e.printStackTrace();
        }*/
        Form f = c.getComponentForm();
        Container messages = (Container) findByName("messages", f);
        final TextArea txtComment = (TextArea) findByName("txtComment", f);
        if (txtComment.getText().length() > 0) {
            String message = txtComment.getText();

            ParseObject newComment = ParseObject.create(Const.PARSE_CLASS_COMMUNICATION);
            ...
            try {
                newComment.put("task", task);
                newComment.put("message", message);
                newComment.put("user", user);
                newComment.put("email", user.getString("email"));
                newComment.put("userName", user.getString("firstName") + " " + user.getString("lastName"));
                newComment.save();
                txtComment.setText("");
    ...
    protected void beforeFormCurrency(Form f) {
        analytics(f, "Currency");
        Hashtable currencyModel = DashboardServices.getCurrencies();

        String dollarText = "1 Dollar = R" + currencyModel.get("dollar");
        String poundText = "1 Pound = R" + currencyModel.get("pound");
        String euroText = "1 Euro = R" + currencyModel.get("euro");

        Button btnDollar = new Button(dollarText);
        Button btnPound = new Button(poundText);
        Button btnEuro = new Button(euroText);
    ...
    protected void beforeFormSocial(Form f) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            MultiButton mb = new MultiButton();
            mb.setUIID("MultiButtonColor");
            mb.setTextLine1(t.getText());
            mb.setTextLine2(t.getScreen_name() + " sent " + t.getCreated_at());

            mb.addActionListener(new ActionListener() {
                @Override
                public void actionPerformed(ActionEvent evt) {
                    Utility.showDialog(t.getScreen_name() + " Tweeted", t.getText() + " sent " + t.getCreated_at());
                }
            });
    ...
    protected void beforeFormWeather(Form f) {
        analytics(f, "Weather");
        java.util.List<Weather> list = DashboardServices.getWeather();

        for (int i = 0; i < list.size(); i++) {
            Weather t = list.get(i);
            MultiButton mb = new MultiButton();
            mb.setUIID("MultiButtonColor");
            mb.setTextLine1(t.getName());
            mb.setTextLine2(t.getTemp() + " degrees " + t.getDescription());

            mb.addActionListener(new ActionListener() {
                @Override
                public void actionPerformed(ActionEvent evt) {
                    //Utility.showDialog(t.getScreen_name() + " Tweeted", t.getText() + " sent " + t.getCreated_at(), "Ok");
                }
            });
...
//    @Override
//    protected boolean onContDashBoardCommand20() {
//        ParseUser loggedInUser = ParseUser.getCurrent();
//        String twitterHandle = loggedInUser.getString("twitterHandle");
//
//        list = DashboardServices.getTweets(twitterHandle);
//
//        if (list.isEmpty()) {
//            Utility.showDialog("Error", "Could not find any Tweets for the handle \"" + twitterHandle + "\". Please go to Settings or try again.");
//            formProgress.removeProgress();
//            return true;
//        }
//        return false;
//    }
    ...
    protected void onUserNew_BtnAddUserAction(Component c, ActionEvent event) {
        ...
        TextField txtFirstname = (TextField) findByName("txtFirstname", f);
        TextField txtLastname = (TextField) findByName("txtLastname", f);
        TextField txtEmail = (TextField) findByName("txtEmail", f);
        TextField txtPassword = (TextField) findByName("txtPassword", f);
        OnOffSwitch isAdmin = (OnOffSwitch) findByName("isAdmin", f);
        ...
        if (message.length() == 0) {
            ...
            ParseUser newUser = ParseUser.create(Const.PARSE_CLASS_USER);
            ...
            newUser.put("isAdmin", isAdmin.isValue());
            newUser.put("twitterHandle", "itweb");

            try {
                formProgress = new FormProgress(f);
                UserService.addUser(newUser);
                formProgress.removeProgress();
                Utility.showDialog("Success", "Added new User.");
                fetchManageContacts(current);
                current.showBack();

            } catch (ParseException e) {
                Utility.showDialog("Error", "Failed to add new User. " + e.getMessage());
                Log.p("e=" + e.getMessage(), Log.DEBUG);
...
//                current.showBack();
//            } else {
//                Utility.showDialog("Error", "Failed to add new User.");
//            }
    ...
    protected void beforeUserNew(Form f) {
        ...
        Toolbar bar = analytics(f, "New User");

//        Button btnDeleteUser = (Button) findByName("btnDeleteUser", f);
//        f.removeComponent(btnDeleteUser);
//        f.repaint();
        Command back = new Command("Home") {
